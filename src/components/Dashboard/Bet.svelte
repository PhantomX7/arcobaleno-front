<script>
	...
	if (browser) {
		$session.refreshTransaction = true;
	}

	$: if (browser && $session.refreshTransaction) {
		$session.refreshTransaction = false;
		(async function () {
			const [response, err] = await runPromise(
				arcobaleno($session).get(`/public/cjk-transaction/summary`),
			);
			if (err) {
				console.log('error when fetching transaction data');
			}
	...
	<form on:submit={handleSubmit}>
		<div class="overflow-auto py-4 px-3 w-full">
							...
							<div class="flex-1 truncate">
								...
								<div class=" pl-2 w-full">
									<Input
										...
										lead="Rp"
										on:change={handleChange}
										value={$form[l] ? formatNumber($form[l].replaceAll('.', '') * 1000) : 0}
									/>
								</div>
							</div>
						</div>
						<div>
							<div class="-mt-px flex divide-x text-sm divide-gray-200">
								...
								>
									...
									<span class="">Rp {formatNumber(transaction[l])}</span>
								</div>
							</div>
						</div>
					</li>
			...
			<div class="text-center flex justify-center">
				<div class="w-full max-w-md">
					<Input
						...
						lead="Rp"
						on:change={handleChange}
						value={formatNumber($session.user.wallet.amount)}
					/>
				</div>
			</div>
			<div class="text-center flex justify-center">
			...
			<div class="text-center mt-4 flex justify-center">
				<Button fullWidth={true} className="max-w-md" type="submit" loading={$isSubmitting}
					>Beli</Button
				>
			</div>
		</div>
